...
def handle_request(req):
    ...
    rospy.loginfo("서비스 요청이 들어왔습니다. 특정 행동을 수행합니다.")
    count=0
    ...
    # 클라이언트에게 응답 메세지를 발행
    response_message = Point()
    response_message.x = 1
    response_message.y = 1
    response_message.z = 1

    rospy.loginfo("응답 메세지를 발행합니다: %s", response_message)
    publisher = rospy.Publisher('response_topic', Point, queue_size=10)
...
if __name__=='__main__':
    ...
    service = rospy.Service('perform_action', Empty, handle_request)
