...
class normal_vector():
    ...
    def fit_plane_to_points_ransac(self, points, threshold=1000, max_iterations=10000):
        # 데이터를 Nx3 크기의 배열로 변환
        ...
        for _ in range(max_iterations):
            ...
            distances = np.abs(A * x + B * y + C * z - D) / np.sqrt(A**2 + B**2 + C**2) / 100
            ...
            if best_distance_sum > distance_sum and A != 0 and A != 1 and B != 0 and B != 1 and C != 0 and C != 1:
                best_plane = (A, B, C, D)
...
posearray = PointArray(topic="/mouse_click_points")
no_vector = normal_vector()
while not rospy.is_shutdown():
    ...
    if posearray.point is None:
        ...
        no_vector.plane_visualization(points_set)
